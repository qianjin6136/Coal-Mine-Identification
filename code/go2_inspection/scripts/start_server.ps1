...
$workspaceVenvPython = Join-Path $workspaceRoot ".venv\Scripts\python.exe"
$env:YOLO_CONFIG_DIR = Join-Path $workspaceRoot "runtime_data"

if (Test-Path -LiteralPath $projectVenvPython) {
    $pythonExe = $projectVenvPython
} elseif (Test-Path -LiteralPath $workspaceVenvPython) {
    $pythonExe = $workspaceVenvPython
} else {
