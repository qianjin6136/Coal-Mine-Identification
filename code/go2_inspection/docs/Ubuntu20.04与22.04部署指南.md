...
建议保留当前目录层级，否则 `configs/app.json` 中的默认运行数据相对路径需要同步
修改：
...
```bash
cd /opt/go2-vision/code/go2_inspection
```
...
以下示例假设项目位于 `/opt/go2-vision/code/go2_inspection`，服务以专用
`go2` 用户运行。先确保该用户可读源码，并可写 `/opt/go2-vision/runtime_data`。
...
User=go2
Group=go2
WorkingDirectory=/opt/go2-vision/code/go2_inspection
Environment=LANG=C.UTF-8
Environment=YOLO_CONFIG_DIR=/opt/go2-vision/runtime_data
ExecStart=/opt/go2-vision/code/go2_inspection/.venv/bin/python -m uvicorn app.api:app --host 0.0.0.0 --port 8000
Restart=on-failure
RestartSec=3
