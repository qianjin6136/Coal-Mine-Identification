...
class StationNumberTemplateModel:
    ...
    def __init__(
        ...
        self._flat_templates = self.templates.reshape(len(values), -1).astype(
            np.float32
        )
        self._template_sums = self._flat_templates.sum(axis=1)
        self.labels = tuple(int(value) for value in labels)
        labels_array = np.asarray(self.labels, dtype=np.int16)
        self._class_template_indices = {
            label: np.flatnonzero(labels_array == label)
    ...
    def predict(self, feature: object) -> tuple[int, float]:
        ...
        distances = np.ones(len(self.templates), dtype=np.float32)
        for shifted in _shifted_station_features(query):
            intersections = self._flat_templates @ shifted.reshape(-1).astype(
                np.float32,
                copy=False,
            )
            totals = self._template_sums + query_sum
            shifted_distances = np.where(
                totals > 0,
                1.0 - (2.0 * intersections / totals),
                1.0,
            )
            distances = np.minimum(distances, shifted_distances)
        class_distances = sorted(
            (
                float(
                    np.mean(
                        np.sort(distances[indices])[:4]
                    )
                ),
                label,
            )
            for label, indices in self._class_template_indices.items()
        )
        best_distance, best_label = class_distances[0]
        second_distance = (
            class_distances[1][0] if len(class_distances) > 1 else 1.0
        )
    ...
    def save(self, path: Path) -> Path:
        ...
        np.savez_compressed(
            ...
            model_version=np.asarray(["station-number-template-v2"]),
        )
        return path
...
def train_station_number_model(
    ...
    *,
    validation_minimum_confidence: float = 0.55,
) -> tuple[StationNumberTemplateModel, dict[str, Any]]:
    """从以编号命名的样本中提取模板，并用确定性扰动做稳健性评估。"""

    import numpy as np

    if not 0.0 <= validation_minimum_confidence <= 1.0:
        raise ValidationError(
            "validation_minimum_confidence must be between 0 and 1"
        )
    ...
    for excluded_index, (expected, source, image) in enumerate(source_images):
        ...
        validation_recognizer = StationNumberRecognizer(
            validation_model,
            minimum_confidence=validation_minimum_confidence,
        )
    ...
    metrics = {
        ...
        "validation_minimum_confidence": validation_minimum_confidence,
...
def segment_station_number(image: object) -> dict[str, Any]:
    ...
    white = cv2.inRange(sign_hsv, (0, 0, 100), (179, 115, 255))
...
def _dice_distance_with_shift(query: object, template: object) -> float:
    ...
    best = 1.0
    for shifted in _shifted_station_features(query_values):
...
def _shifted_station_features(feature: object) -> list[object]:
    import numpy as np

    values = np.asarray(feature, dtype=np.uint8)
    shifted_features: list[object] = []
    for shift_y, shift_x in (
        ...
        ] = values[source_y1:source_y2, source_x1:source_x2]
